...
const Statistics: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      <h1 className="text-3xl font-bold mb-6 text-violet-700">Görev İstatistikleri</h1>
      <div className="flex gap-4 mb-6">
        {FILTERS.map(f => (
          <button
            key={f.key}
            onClick={() => setDateFilter(f.key)}
            className={`px-4 py-2 rounded-xl font-medium border transition-all duration-200 ${dateFilter === f.key ? 'bg-gradient-to-r from-violet-500 to-purple-600 text-white border-violet-500' : 'bg-white text-violet-700 border-violet-200 hover:bg-violet-50'}`}
          >
            {f.label}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col items-center w-full">
          <h2 className="text-lg font-semibold mb-4 text-slate-700">Görev Durum Dağılımı</h2>
          <div className="w-full h-80 relative">
            {hasBarData ? (
              <>
                ...
                <div className="absolute inset-x-0 bottom-1 flex items-center justify-center gap-2 px-1 flex-wrap text-sm">
        ...
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col items-center w-full">
          <h2 className="text-lg font-semibold mb-4 text-slate-700">Oranlar</h2>
      ...
      <div className="bg-white rounded-2xl shadow p-6 flex flex-col md:flex-row gap-8 items-center justify-between">
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="text-slate-600">Devam Eden Görev</span>
        </div>
        <div className="flex flex-col items-center">
          <span className="text-2xl font-bold text-orange-600">{todoCount}</span>
          <span className="text-slate-600">Yapılacak Görev</span>
        </div>
        <div className="flex flex-col items-center">
          <span className="text-2xl font-bold text-violet-700">{filteredTasks.length}</span>
